...
subtest 'list() tests' => sub {
    ...
    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/ledgers")->status_is(200)->json_is( [] );
    ...
    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/ledgers")->status_is(200)
        ->json_is( [ $ledger->to_api, $another_ledger->to_api, ] );

    # Attempt to search by title like 'ko'
    $ledger->delete;
    $another_ledger->delete;
    $t->get_ok(qq~//$userid:$password@/api/v1/contrib/acquire/ledgers?q=[{"me.code":{"like":"%ko%"}}]~)->status_is(200)
        ->json_is( [] );
    ...
    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/ledgers?blah=blah")->status_is(400)
...
subtest 'get() tests' => sub {
    ...
    $t->get_ok( "//$userid:$password@/api/v1/contrib/acquire/ledgers/" . $ledger->ledger_id )->status_is(200)
        ->json_is( $ledger->to_api );

    # Attempt to get non-existent ledgers
    my $non_existent_ledgers = $builder->build_object( { class => 'Koha::Acquire::Funds::Ledgers' } );
    my $non_existent_id     = $non_existent_ledgers->ledger_id;
    $non_existent_ledgers->delete;

    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/ledgers/$non_existent_id")->status_is(404)
...
subtest 'add() tests' => sub {
    ...
    my $ledger_id =
        $t->post_ok( "//$userid:$password@/api/v1/contrib/acquire/ledgers" => json => $ledger )
        ->status_is( 201, 'SWAGGER3.2.1' )->header_like(
        Location => qr|^/api/v1/contrib/acquire/ledgers/\d*|,
        'SWAGGER3.4.1'
    )->json_is( '/name' => $ledger->{name} )->json_is( '/description' => $ledger->{description} )
        ->json_is( '/code'     => $ledger->{code} )->json_is( '/status' => $ledger->{status} )
        ->json_is( '/owner_id' => $ledger->{owner_id} )->json_is( '/visible_to' => $ledger->{visible_to} )
...
subtest 'update() tests' => sub {
    ...
    my $ledger_id = $builder->build_object( { class => 'Koha::Acquire::Funds::Ledgers' } )->ledger_id;

    # Full object update on PUT
    my $ledgers_with_updated_field = {
        name        => "new name",
        description => "test",
        code        => "1",
        external_id => "01",
    ...
    $t->put_ok(
        "//$userid:$password@/api/v1/contrib/acquire/ledgers/$ledger_id" => json => $ledgers_with_updated_field )
        ->status_is(200)->json_is( '/name' => 'new name' );

    # Attempt to update non-existent ledgers
    my $ledgers_to_delete = $builder->build_object( { class => 'Koha::Acquire::Funds::Ledgers' } );
    my $non_existent_id   = $ledgers_to_delete->ledger_id;
    $ledgers_to_delete->delete;

    $t->put_ok(
        "//$userid:$password@/api/v1/contrib/acquire/ledgers/$non_existent_id" => json => $ledgers_with_updated_field )
        ->status_is(404);

    # Wrong method (POST)
    $ledgers_with_updated_field->{ledger_id} = 2;
...
subtest 'delete() tests' => sub {
    ...
    $t->delete_ok("//$userid:$password@/api/v1/contrib/acquire/ledgers/$ledger_id")->status_is( 204, 'SWAGGER3.2.4' )
        ->content_is( '', 'SWAGGER3.3.4' );

    # Attempt to delete non-existent ledgers
    $t->delete_ok("//$userid:$password@/api/v1/contrib/acquire/ledgers/$ledger_id")->status_is(404);
