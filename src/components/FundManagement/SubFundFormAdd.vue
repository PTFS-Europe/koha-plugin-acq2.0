                        ...
                        <li>
                            ...
                            >
                                <template #search="{ attributes, events }">
                                    <input
                                        :required="!statusOptions.map(opt => opt.value).includes(sub_fund.status)"
                                        class="vs__search"
                                        v-bind="attributes"
                                        v-on="events"
                                    />
    ...
    methods: {
        ...
        onSubmit(e) {
            ...
            if(!this.isUserPermitted('createFund')) {
                setWarning('You do not have the required permissions to create sub funds.')
                return
            }
            ...
            if(sub_fund_id) {
                ...
                acq_client.subFunds
                    .update(sub_fund, sub_fund_id).then(
                        success => {
                            setMessage("Sub fund updated")
                            this.$router.push({ name: "FundShow", params: { fund_id: sub_fund.fund_id} })
                        },
                        error => {}
                    )
            } else {
