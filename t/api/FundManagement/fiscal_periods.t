...
subtest 'list() tests' => sub {
    ...
    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/fiscal_periods")->status_is(200)->json_is( [] );

    my $fiscal_period = $builder->build_object(
        { class => 'Koha::Acquire::Funds::FiscalPeriods', value => { visible_to => $lib_group->id } } );

    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/fiscal_periods")->status_is(200)
        ->json_is( [ $fiscal_period->to_api ] );

    my $another_fiscal_period = $builder->build_object(
        {
            class => 'Koha::Acquire::Funds::FiscalPeriods',
    ...
    $t->get_ok(qq~//$userid:$password@/api/v1/contrib/acquire/fiscal_periods?q=[{"me.code":{"like":"%ko%"}}]~)
    ...
    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/fiscal_periods?blah=blah")->status_is(400)
        ->json_is( [ { path => '/query/blah', message => 'Malformed query string' } ] );

    # # Unauthorized access
    # $t->get_ok("//$unauth_userid:$password@/api/v1/contrib/acquire/fiscal_periods")->status_is(403);
...
subtest 'get() tests' => sub {
    ...
    $t->get_ok( "//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/" . $fiscal_period->fiscal_period_id )
        ->status_is(200)->json_is( $fiscal_period->to_api );

    # # Unauthorized access
    # $t->get_ok( "//$unauth_userid:$password@/api/v1/contrib/acquire/fiscal_periods/"
    #         . $fiscal_period->fiscal_period_id )->status_is(403);

    # Attempt to get non-existent fiscal_period
    my $non_existent_fiscal_period = $builder->build_object( { class => 'Koha::Acquire::Funds::FiscalPeriods' } );
    my $non_existent_id           = $non_existent_fiscal_period->fiscal_period_id;
    $non_existent_fiscal_period->delete;

    $t->get_ok("//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/$non_existent_id")->status_is(404)
...
subtest 'add() tests' => sub {
    ...
    # $t->post_ok( "//$unauth_userid:$password@/api/v1/contrib/acquire/fiscal_periods" => json => $fiscal_period )
    #     ->status_is(403);
    ...
    # $t->post_ok(
    #     "//$userid:$password@/api/v1/contrib/acquire/fiscal_periods" => json => $fiscal_period_with_invalid_field )
    ...
    my $fiscal_period_id =
        $t->post_ok( "//$userid:$password@/api/v1/contrib/acquire/fiscal_periods" => json => $fiscal_period )
        ->status_is( 201, 'SWAGGER3.2.1' )->header_like(
        Location => qr|^/api/v1/contrib/acquire/fiscal_periods/\d*|,
        'SWAGGER3.4.1'
        ...
        ->json_is( '/visible_to' => $fiscal_period->{visible_to} )->tx->res->json->{fiscal_period_id};
...
subtest 'update() tests' => sub {
    ...
    my $fiscal_period_id =
        $builder->build_object( { class => 'Koha::Acquire::Funds::FiscalPeriods' } )->fiscal_period_id;

    # Unauthorized attempt to update
    # $t->put_ok( "//$unauth_userid:$password@/api/v1/contrib/acquire/fiscal_periods/$fiscal_period_id" => json =>
    #         { name => 'New unauthorized name change' } )->status_is(403);

    # Attempt partial update on a PUT
    # my $fiscal_period_with_missing_field = {
    #     description => "test",
    #     start_date  => "2024-01-01",
    #     end_date    => "2025-01-01",
    #     status      => "1",
    ...
    # $t->put_ok( "//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/$fiscal_period_id" => json =>
    #         $fiscal_period_with_missing_field )->status_is(400)
    ...
    my $fiscal_period_to_delete = $builder->build_object( { class => 'Koha::Acquire::Funds::FiscalPeriods' } );
    my $non_existent_id         = $fiscal_period_to_delete->fiscal_period_id;
    $fiscal_period_to_delete->delete;

    $t->put_ok( "//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/$non_existent_id" => json =>
            $fiscal_period_with_updated_field )->status_is(404);

    # Wrong method (POST)
    $fiscal_period_with_updated_field->{fiscal_period_id} = 2;

    $t->post_ok( "//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/$fiscal_period_id" => json =>
            $fiscal_period_with_updated_field )->status_is(404);
...
subtest 'delete() tests' => sub {
    ...
    # $t->delete_ok("//$unauth_userid:$password@/api/v1/contrib/acquire/fiscal_periods/$fiscal_period_id")
    #     ->status_is(403);

    # Delete existing fiscal_period
    $t->delete_ok("//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/$fiscal_period_id")
        ->status_is( 204, 'SWAGGER3.2.4' )->content_is( '', 'SWAGGER3.3.4' );

    # Attempt to delete non-existent fiscal_period
    $t->delete_ok("//$userid:$password@/api/v1/contrib/acquire/fiscal_periods/$fiscal_period_id")->status_is(404);
